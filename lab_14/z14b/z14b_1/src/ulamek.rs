...
impl Ulamek {
    ...
    pub fn new_single(licznik: i64) -> Self {
        Ulamek::new(licznik,1)
    }
...
mod tests {
    ...
    #[test]
    fn test_from() {
        let u1: Ulamek = Ulamek::from(7);
        let u2: Ulamek = Ulamek::from((4, 5));
        let u3: Ulamek = Ulamek::from(1.5);
        let u4: Ulamek = Ulamek::from("1/-3");
        assert_eq!(u1,Ulamek::new(7,1));
        assert_eq!(u2,Ulamek::new(4,5));
        assert_eq!(u3,Ulamek::new(3,2));
        assert_eq!(u4,Ulamek::new(-1,3));

    }
}

impl From<i64> for Ulamek {
    fn from(licznik: i64) -> Self {
        Ulamek::new_single(licznik)
    }
}

impl From<(i64,i64)> for Ulamek {
    fn from(tuple: (i64,i64)) -> Self {
        Ulamek::new(tuple.0,tuple.1)
    }
}

impl From<f64> for Ulamek {
    fn from(liczba: f64) -> Self {
        let mut mianownik = 1;
        let mut licznik = liczba;
        while licznik.fract() != 0.0 {
            licznik *= 2.0;
            mianownik *=2;
        }
        Ulamek::new(licznik as i64, mianownik)
    }
}

impl From<&str> for Ulamek {
    fn from(s: &str) -> Self {
        Ulamek::from_str(s).expect("Niepoprawny format ulamka")
    }
}